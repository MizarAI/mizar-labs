...
from mizarlabs.transformers.technical.moving_average import (
    ExponentialWeightedMovingAverageDifference,
)
...
def test_exponential_weighted_moving_average_difference(
    ...
    fast = 10
    slow = 20
    ewma_diff_norm = ExponentialWeightedMovingAverageDifference(
        fast=fast, slow=slow, column_name="close", normalised=True
    )
    ewma_diff_unnorm = ExponentialWeightedMovingAverageDifference(
        fast=fast, slow=slow, column_name="close", normalised=False
    )
